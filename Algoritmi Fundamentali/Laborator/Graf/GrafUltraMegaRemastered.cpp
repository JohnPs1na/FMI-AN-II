...
{
    out<<e.id<<") "<<e.source<<' '<<e.destination<<' '<<e.cost<<'\n';
    return out;
}
...
private:
    ...
    void ROYFLOYD(vector<vector<int>>& matrix_of_weight,vector<vector<int>>& max_intersections);
...
public:
    ...
    vector<vector<int>> solve_roy_floyd(vector<vector<int>>& matrix_of_weights,vector<vector<int>>& max_intersections);
    ...
    //ex infoarena
    void solve_marmeladka(int S,int D);
    void df_s_revni(int node,int cost, vector<int>& answer);
};
...
int main()
{
    ...
    vector<vector<int>> matrix_of_weights(N+1,vector<int>(N+1,0));
    vector<vector<int>> max_intersections(N+1,vector<int>(N+1,1));

    for(int i = 1;i<=N;i++){
        for(int j = 1;j<=N;j++)
            fin>>matrix_of_weights[i][j];
        max_intersections[i][i] = 0;
    }
        
    Graph g(N);

    g.solve_roy_floyd(matrix_of_weights,max_intersections);
...
void Graph::infoarena_graph() {
    ...
    for(int i = 1;i<=edges;i++)
    {
        ...
        if(!oriented)
            adjacency_list[y].push_back(Edge(y,x,c,i));
    }
}
...
void Graph::show_my_graph() {
    ...
    for(int i = 1;i<=vertices;i++){
        cout<<i<<"=>";
        for(auto path : adjacency_list[i])
            cout<<"(id-"<<path.id<<". "<< path.destination<<", "<<"Cost: " << path.cost<<')' << ", ";
        cout<<'\n';
    }
...
void Graph::BFS(int starting_vertex,vector<int>& distances)
{
    ...
    vector<pair<int,int>> parents(vertices+1);
    queue<int> que;
...
void Graph::ROYFLOYD(vector<vector<int>>& matrix_of_weights,vector<vector<int>>& max_intersections)
{
    ...
    for(int k = 1;k<=vertices;k++)
        for(int i = 1;i<=vertices;i++)
            ...
            {
                if(matrix_of_weights[i][j] == matrix_of_weights[i][k] + matrix_of_weights[k][j])
                    max_intersections[i][j] = max(max_intersections[i][j],max_intersections[i][k] + max_intersections[k][j]);
                else if(matrix_of_weights[i][j] > matrix_of_weights[i][k] + matrix_of_weights[k][j]){
                    matrix_of_weights[i][j] = matrix_of_weights[i][k] + matrix_of_weights[k][j];
                    max_intersections[i][j] = max_intersections[i][k] + max_intersections[k][j];
                }
            }
...
vector<vector<int>> Graph::solve_roy_floyd(vector<vector<int>>& matrix_of_weights,vector<vector<int>>& max_intersections)
{
    ROYFLOYD(matrix_of_weights,max_intersections);
...
void Graph::solve_marmeladka(int S,int D)
{
    vector<int> distances(vertices+1,-1);
    vector<pair<int,int>> parents(vertices+1);
    vector<pair<int,int>> costs(edges+1);
    vector<int> answer(edges+1,0);
    ...
    while(!que.empty()){
        ...
        for(auto path : adjacency_list[vert])
            if(distances[path.destination] == -1){
                que.push(path.destination);
                distances[path.destination] = distances[vert] + 1;
                parents[path.destination] = {path.source,path.id};
            }
    }
    ...
    sort(costs.begin()+1,costs.begin()+edges+1);


    for(int node = D,idx = distances[node]; node!=S;node = parents[node].first,idx--)
        answer[parents[node].second] = costs[idx].second;

    int idx = distances[D] + 1;

    for(int i = 1;i<=edges;i++)
        if(!answer[i]){
            answer[i] = costs[idx].second;
            idx++;
        }
...
void Graph::df_s_revni(int node,int cost,vector<int>& answer)
{
    answer[node] = cost;
    for(auto path : adjacency_list[node]){
        if(!answer[path.destination])
            df_s_revni(path.destination,cost, answer);
    }
