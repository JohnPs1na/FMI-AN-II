...
Presupunem ca avem costul de mutare al unui bloc egal cu indicele in alfabet, cu indicii incepănd de la 1
(care se calculează prin 1+ diferenta dintre valoarea codului ascii al literei blocului de mutat si codul ascii al literei "a" ) .
 Astfel A* are trebui sa prefere drumurile in care se muta intai blocurile cu infomatie mai mica lexicografic
 pentru a ajunge la una dintre starile scop
"""
...
#13
    def testeaza_scop_egale(self,nodCurent):
        if not nodCurent:
            return False
        sol = len(nodCurent.info[0])
        for i in nodCurent.info:
            if len(i) != sol:
                return False
        return True

    def valid_stive_egale(self):
        elements = []
                    ...
                    for iElem, elem in enumerate(stiva):
                        try:
                            ...
                            if elem != scop[iStiva][iElem]:
                                h += 1 + ord(scop[iStiva][iElem]) - ord('a') # adun cu costul minim pe o mutare (adica costul lui a)
                        except IndexError:
                ...
                for iStiva, stiva in enumerate(infoNod):
                    for iElem, elem in enumerate(stiva):
                        try:
                            # exista în stiva scop indicele iElem dar pe acea pozitie nu se afla blocul din infoNod
                            if elem != scop[iStiva][iElem]:
                                h += 1 + ord(scop[iStiva][iElem]) - ord('a')
                            else:  # elem==scop[iStiva][iElem]:
                                # if stiva[:iElem] != scop[iStiva][:iElem]:
                                #     h += 2
                                for index in range(iElem):
                                    if stiva[index] != scop[iStiva][index]:
                                        h += 1 + ord(stiva[index]) - ord('a')
                        except IndexError:
                            # nici macar nu exista pozitia iElem in stiva cu indicele iStiva din scop
                            h += 1
                euristici.append(h)
...
def a_star_stive_egale(gr, nrSolutiiCautate, tip_euristica):
    # in coada vom avea doar noduri de tip NodParcurgere (nodurile din arborele de parcurgere)
    c = [NodParcurgere(gr.start, None, 0, gr.calculeaza_h(gr.start))]
    if not gr.valid_stive_egale():
        return False

    while len(c) > 0:
        nodCurent = c.pop(0)

        if gr.testeaza_scop_egale(nodCurent):
            print("Solutie: ")
            nodCurent.afisDrum(afisCost=True, afisLung=True)
            print("\n----------------\n")
            input()
...
nrSolutiiCautate = 3
# a_star(gr, nrSolutiiCautate=3, tip_euristica="euristica admisibila 2")

a_star_stive_egale(gr, nrSolutiiCautate=3, tip_euristica="euristica admisibila 2")
"""
a b c
d e
g
